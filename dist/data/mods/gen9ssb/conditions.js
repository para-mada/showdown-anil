...
const Conditions = {
  ...
  */
  ...
  imperial: {
    noCopy: true,
    onStart() {
      this.add(`c:|${(0, import_scripts.getName)("Imperial")}|gmcat`);
    },
    onSwitchOut(pokemon) {
      const foe = pokemon.side.foes()[0]?.name;
      if (foe) {
        this.add(`c:|${(0, import_scripts.getName)("Imperial")}|ofc u have ${foe}. bad mu as always...`);
      }
    },
    onFaint() {
      this.add(`c:|${(0, import_scripts.getName)("Imperial")}|crazy rng`);
    }
  },
  inthehills: {
    noCopy: true,
