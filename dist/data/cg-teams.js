...
class TeamGenerator {
  ...
  makeSet(species, teamStats) {
    ...
    while (moves.length < 4 && movePool.length) {
      ...
      if (move.category === "Status") {
        ...
        if ((movesStats.attackTypes[moveType] || 0) < bp)
    ...
    if (species.forceTeraType) {
      ...
      if (hasTeraBlast || hasRevelationDance || noAttacks) {
        ...
        if (noStellar)
          attackingTypes.splice(attackingTypes.indexOf("Stellar"));
      } else {
        if (!noStellar)
          attackingTypes.push("Stellar");
      }
  ...
  getItemWeight(item, teamStats, species, moves, ability, level) {
    ...
    switch (item.id) {
      ...
      case "choicescarf":
        if (moves.some((x) => x.category === "Status" || x.secondary?.self?.boosts?.spe || x.priority > 1))
          return 0;
